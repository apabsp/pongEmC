...
void startMyPaddle(struct barrinha **head, int startingPositionX, int startingPositionY, int height, char color) { // recommended height is 6
    *head = (struct barrinha *)malloc(sizeof(struct barrinha));
    ...
    for (int i = 1; i < height; i++) {
        ...
        if ( i <= dividedHeight){
            //descomente pra ver a representaç~ao por direç~ao
            meuNovo->direction = 'u';
            //strcpy(current->representationAndColor,"u");

        } else if(i >= dividedHeight*2){
            meuNovo->direction = 'd';
            //strcpy(current->representationAndColor,"d");
        }
        else{
            meuNovo->direction = 'm';
            //strcpy(current->representationAndColor,"m");
        }
